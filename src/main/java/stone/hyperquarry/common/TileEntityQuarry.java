...
public class TileEntityQuarry extends TileEntity implements ITickable, IEnergyStorage {
    ...
    public void update() {
        ...
        if (maybeEntity != null) {
            IItemHandler handler = maybeEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
                    EnumFacing.DOWN);
            if (handler != null) {
    ...
    private void refundEnergy(int stacks, int leftover) {
        ...
        try {
            this.energy = Math.addExact(this.energy,
                    Math.multiplyExact(stacks * Byte.MAX_VALUE + leftover, this.drops.cost()));
    ...
    public void onLoad() {
        ...
        if (this.drops == null)
            this.drops = DropList.of(new Filter("drops", 0, new BitSet()));
        if (isRunning) {
